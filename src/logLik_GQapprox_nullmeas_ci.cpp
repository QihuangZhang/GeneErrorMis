...
// [[Rcpp::export]]
double logLik_GQapprox_nullmeas_ci(NumericVector w, NumericVector v, NumericVector Y1star, NumericVector Y2star, NumericMatrix Covariates, int isY2in, NumericMatrix CovMis2, NumericVector R, 
  ...
  for(k = 0; k < nS; ++k){
    for(i = 0; i < nsam; ++i)
    { eta1all(i) = eta1(i) + lambda(0) * sqrt(2*R(i)) * sigma_g * v(k);
      eta2exp = eta2(i) + lambda(1) *  sqrt(2*R(i)) * sigma_g * v(k);
