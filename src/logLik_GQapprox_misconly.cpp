...
// [[Rcpp::export]]
double logLik_GQapprox_misconly(NumericVector w, NumericVector v, NumericVector Y1star, NumericVector Y2star, NumericMatrix Covariates, int isY2in, NumericMatrix CovMis2, NumericVector R, 
  ...
  double eta2exp,fip,f2i1,f2i0,f1i,fi;
  ...
  for(k = 0; k < nS; ++k){
    ...
    for(i = 0; i < nsam; ++i){
        ...
        fip = f1i * (f2i0 * (1-eta2all(i)) + f2i1 * eta2all(i));
