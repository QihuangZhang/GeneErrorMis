...
// [[Rcpp::export]]
double logLik_twin_nlam(NumericVector w, NumericVector v, NumericVector Y1star, NumericVector Y2star, NumericMatrix Covariates, int isY2in, NumericMatrix CovMis2, double rho, 
  ...
  double eta2exp,f2i1,f2i0,f1i1,f1i0,fi,fip1,fip2;
        ...
        for(t = 0; t < nS; ++t){
          for (r = 0; r < nS; ++r){
            for(i = 0; i < nsam/2; ++i){
              ...
              fip1 = f1i0 * f2i0 * (1-eta2all(i)) + f1i1 * f2i1 * eta2all(i);
              ...
              Y2 =1;
              if (isY2in == 1){
                mta_1(i+nsam/2) += gamma * Y2;
              } else {
                mta_1(i+nsam/2) += gamma * (2 * Y2 - 1);
              }
              f1i1 = ( 1 / (fabs(sigma_e) * sqrt(2*M_PI)) ) * exp( -0.5 * pow( (Y1star(i+nsam/2)-mta_1(i+nsam/2))/fabs(sigma_e), 2.0 ));
              ...
              f1i0 = ( 1 / (fabs(sigma_e) * sqrt(2*M_PI)) ) * exp( -0.5 * pow( (Y1star(i+nsam/2)-mta_1(i+nsam/2))/fabs(sigma_e), 2.0 ));
              ...
              fip2 = f1i0 * f2i0 * (1-eta2all(i+nsam/2)) + f1i1 * f2i1 * eta2all(i+nsam/2);
              
              fkkt(i) +=  w(t)*w(k)*w(l)*w(s)*w(r)*fip1*fip2 *weightPi;
            }
          }
