...
// [[Rcpp::export]]
double logLikelihood_indep_c(NumericMatrix u, NumericMatrix random1, NumericMatrix random2, NumericVector Y1star, NumericVector Y2star, NumericMatrix Covariates, NumericMatrix CovMea1, int isY2in, NumericMatrix CovMis2,  
                                    NumericVector beta1, NumericVector beta2, NumericVector lambda, NumericVector gamma, NumericVector alpha1, NumericVector alpha0, double sigma, double sigma_e){
  int i,j,k,t;
  int nk=u.nrow();
  int nrepeat=random1.ncol();
  int nsam=Covariates.nrow();
  int ncov=Covariates.ncol();
  int ncovMea=CovMea1.ncol();
  ...
  for(k = 0; k < nk; ++k){
    ...
    for(t = 0; t < nrepeat; ++t){
      for(i = 0; i < nsam; ++i){
        ...
        mta_1(i)=0;
        for (j = 0; j < ncovMea; ++j){
          mta_1(i) += gamma(j) * CovMea1(i,j) +  Y1(i);
        }
        if ((ngamma-ncovMea)==1) {
          mta_1(i) += gamma(ngamma-1) * Y2(i) * isY2in;
        }
        
        if (Y2(i)==1) {f2i = pow(mta_21expit(i), Y2star(i)) * pow(1-mta_21expit(i), 1-Y2star(i)) ;}
