...
// [[Rcpp::export]]
NumericMatrix IL_infomat_twin(NumericVector w, NumericVector v, NumericVector Y1star, NumericVector Y2star, NumericMatrix Covariates, int isY2in, NumericMatrix CovMis2,  double  rho, 
    ...
    for(k = 0; k < nS; ++k){
      for(l = 0; l < nS; ++l){
        for(s = 0; s < nS; ++s){
          ...
          for(j = 0; j < nS; ++j){
            for(r = 0; r < nS; ++r){
              weightjk = w(k)*w(l)*w(s)*w(j)*w(r)/M_PI;
