...
// [[Rcpp::export]]
NumericMatrix IL_infomat_measonly(NumericVector w, NumericVector v, NumericVector Y1star, NumericVector Y2star, NumericMatrix Covariates, int isY2in, NumericMatrix CovMis2,  NumericVector R, 
    ...
    for(k = 0; k < nS; ++k){
      ...
      for(j = 0; j < nS; ++j){
        ...
        for(t = 0; t< beta2.length(); ++t){
          scores(i,indexbeta1+t) += weightjk * Covariates(i,t) * (Y2star(i)   - eta2all(i)) * fip;
        }
        scores(i,indexgamma-1) += weightjk * (Y1star(i)  - mta_1) * fip * Cov_meas11  / pow(sigma_e,2);
        scores(i,indexsigma-1) += weightjk * (-1/sigma + pow((Y1(i) - eta1all(i)),2)/pow(sigma,3))* fip;
        scores(i,indexsigmae-1) += weightjk * (-1/sigma_e  + pow((Y1star(i) - mta_1),2))* fip/pow(sigma_e,3);
