...
// [[Rcpp::export]]
Rcpp::List EMGQ_measonly(NumericVector w, NumericVector v, NumericVector Y1star, NumericVector Y2star, NumericMatrix Covariates, int isY2in, NumericMatrix CovMis2, NumericVector R,  
  ...
  NumericVector scores(indexsigmag),scoresum(indexsigmag),score(indexsigmag);
    ...
    for(k = 0; k < nS; ++k){
      ...
      for(j = 0; j < nS; ++j){
        ...
        for(t = 0; t < beta1.length(); ++t){
          score(t) = Covariates(i,t) * random1 / pow(sigma,2);
        }
        for(t = 0; t< beta2.length(); ++t){
          score(indexbeta1+t) = Covariates(i,t) * (Y2star(i) - eta2all(i)) ;
        }
        score(indexbeta2) = uk * random1/ pow(sigma,2);
        score(indexgamma-1) =  ((Y1star(i)  - mta_1)* Cov_meas11)  / pow(sigma_e,2);
        score(indexsigma-1) =  -1/sigma + pow((Y1(i) - eta1all(i)),2)/pow(sigma,3);
        score(indexsigmae-1) =  -1/sigma_e +  pow((Y1star(i) - mta_1),2)/pow(sigma_e,3);
        score(indexsigmag-1) =  -1/sigma_g + uk*uk/pow(sigma_g,3)/R(i);
        
        weightjk = w(k)*w(j)/M_PI;
        fkkt(i) +=  weightjk*fip;
        for(t = 0; t < indexsigmag; ++t){
          scores(t) += weightjk * score(t) * fip;
          for (s = 0; s < indexsigmag; ++s){
            FisherInfo_i(t,s) += weightjk * (score(t) * score(s) * fip);
          }
        }
